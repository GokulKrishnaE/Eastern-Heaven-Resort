...
// include html
  ...
  new Swiper('.hero-background-wrapper', {
    speed: 400,
    loop: true,
    autoplay: {
      delay: 5000,
  ...
  const observerCallback = (entries) => {
    entries.forEach((entry) => {
      const subElements = entry.target.querySelectorAll('.room-image');
  
      if (entry.isIntersecting) {
        // Add transform and make it visible when the item intersects
        subElements.forEach(subItem=>{
  ...
  // Create the observer with default options
  const observer = new IntersectionObserver(observerCallback, {
    threshold: 0.5, // Trigger when 50% of the item is visible
  });
  
  // Observe each item
  observer.observe(document.querySelector('.rooms-section'));
